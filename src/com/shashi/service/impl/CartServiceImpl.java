...
public class CartServiceImpl implements CartService {
	...
	public int getCartItemCount(String userId, String itemId) {
        ...
        public void saveSession(String sessionId) {
            ...
            try {
                    ps = conn.prepareStatement("insert into guestcart (sessionId, productId, quantity) values (?, NULL, 0)");

                    ps.setString(1, sessionId);

                    int k = ps.executeUpdate();
                    if (k > 0) {
                        System.out.println("Update session successfully");
                    } else {
        ...
        public int countGuestCart(String sessionId) throws SQLException{
            int count = 0;
            ...
            try {
                ps = conn.prepareStatement("select sum(quantity) from guestcart where sessionId=?");
    ...
    public String updateProductToGuestCart(String sessionId, String prodId, int prodQty) {
        String status = "Failed to Add into Guest Cart";
        ...
        try {
            ...
            ps = con.prepareStatement("select * from guestcart where sessionId=? and productId=?");
            ...
            if (rs.next()) {
                ...
                if (prodQty > 0) {
                    ps2 = con.prepareStatement("update guestcart set quantity=? where sessionId=? and productId=?");
                    ...
                    if (k > 0) {
                        status = "Product Successfully Updated to Guest Cart!";
                    }
                } else if (prodQty == 0) {
                    ps2 = con.prepareStatement("delete from guestcart where sessionId=? and productId=?");
                    ...
                    if (k > 0) {
                        status = "Product Successfully Updated in guest Cart!";
                    }
                }
            } else {

                ps2 = con.prepareStatement("insert into guestcart values(?,?,?)");
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (prodQuantity > 0) {
                    ...
                    if (k > 0) {
                        status = "Product Successfully removed from guest Cart!";
                    }
                } else if (prodQuantity <= 0) {
                    ...
                    if (k > 0) {
                        status = "Product Successfully removed from the guest Cart!";
                    }
                }

            } else {

                status = "Product Not Available in guest cart!";
    ...
    public List<CartBean> getAllGuestCartItems(String sessionId) {
        List<CartBean> items = new ArrayList<CartBean>();
        ...
        try {
            ...
            ps = con.prepareStatement("select * from guestcart where sessionId=? AND productId IS NOT NULL");
            ...
            while (rs.next()) {
                ...
                cart.setUserId(rs.getString("sessionId"));
    ...
    public String addProductToGuestCart(String sessionId, String prodId, int prodQty) {
        String status = "Failed to Add into Cart";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (availableQty < prodQty) {
                    ...
                    status = updateProductToGuestCart(sessionId, prodId, availableQty);

                    status = "Only " + availableQty + " no of " + product.getProdName()
                            + " are available in the shop! So we are adding only " + availableQty
                            + " no of that item into Your Cart" + "";

                    DemandBean demandBean = new DemandBean(sessionId, product.getProdId(), prodQty - availableQty);
                    ...
                    status = updateProductToGuestCart(sessionId, prodId, prodQty);
    ...
    public int getGuestCartItemCount(String sessionId, String itemId) {
        int count = 0;
        if (itemId == null) {
            return 0;
        }
        ...
        try {
            ps = con.prepareStatement("select quantity from guestcart where sessionId=? and productId=?");
